  ...
  return (
    <div>
      <div className="flex flex-col items-center text-center mb-5">
        <div className="p-2 bg-gradient-to-b from-primaryColor-500/20 to-purple-500/10 rounded-2xl border border-black/10 dark:border-white/15 mb-3 shadow-xl shadow-primaryColor-500/10 flex items-center justify-center">
          <img
        ...
        <h2 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-white">{t('unlockVault')}</h2>
        <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
          Enter PIN to decrypt <strong className="text-zinc-800 dark:text-zinc-200">{activeVault?.name || 'Vault'}</strong>
        </p>
      </div>

      {/* Lockout Banner */}
      {isLockedOut && (
        <div className="mb-4 p-3.5 rounded-xl bg-red-500/15 border border-red-500/30 text-red-800 dark:text-red-200 text-xs flex items-center gap-2.5 font-mono animate-in fade-in duration-150">
          <AlertTriangle className="w-4 h-4 text-red-500 shrink-0" />
          <div className="flex-1">
            <div className="font-semibold text-red-700 dark:text-red-300">PIN Entry Suspended</div>
      ...
      <form onSubmit={handlePinUnlock} autoComplete="off" className="space-y-4">
        ...
        <div className="p-3 rounded-xl bg-black/5 dark:bg-black/40 border border-black/10 dark:border-white/10 space-y-1.5">
          <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
            <span className="flex items-center gap-1.5">
              <Database className="w-3.5 h-3.5 text-primaryColor-600 dark:text-primaryColor-400" />
            ...
            <button
              ...
              className="text-[11px] text-primaryColor-600 dark:text-primaryColor-400 hover:text-primaryColor-700 dark:hover:text-primaryColor-300 flex items-center gap-1 font-mono cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="flex-1 min-w-0 px-3 py-2 rounded-lg bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/10 text-xs font-mono text-zinc-900 dark:text-white focus:outline-none focus:border-primaryColor-500/50 transition cursor-pointer"
            >
              {vaults.map((v) => (
            ...
            {onDeleteVault && activeVault && (
              <button
                ...
                className="p-2 rounded-lg bg-black/5 dark:bg-white/5 hover:bg-red-500/15 dark:hover:bg-red-500/20 text-zinc-500 dark:text-zinc-400 hover:text-red-600 dark:hover:text-red-400 border border-black/10 dark:border-white/10 transition cursor-pointer shrink-0"
                title={t('deleteVault')}
              >
        ...
        {confirmDelete && activeVault && (
          <div className="p-3 rounded-xl border border-red-500/40 bg-red-500/10 space-y-2 animate-in fade-in duration-150 text-xs font-mono">
            <div className="flex items-center justify-between text-red-800 dark:text-red-200">
              ...
              <span className="text-[11px] text-red-700 dark:text-red-300 font-sans">{t('confirmDeleteVault')}</span>
            </div>
            <div className="flex items-center gap-2 justify-end pt-1">
              ...
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-black/10 dark:bg-white/10 hover:bg-black/15 dark:hover:bg-white/15 text-zinc-700 dark:text-zinc-300 text-xs transition cursor-pointer"
              >
                {t('cancel')}
              </button>
            </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="••••"
            className="w-full px-4 py-3 rounded-xl bg-black/5 dark:bg-black/40 border border-black/10 dark:border-white/10 text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 focus:outline-none focus:border-primaryColor-500 text-lg tracking-[0.3em] font-mono text-center disabled:opacity-40 disabled:cursor-not-allowed"
            required
            autoFocus
          />
        </div>
      ...
      <div className="mt-4 text-center">
        <button
          ...
          className="text-xs text-primaryColor-600 dark:text-primaryColor-400 hover:text-primaryColor-700 dark:hover:text-primaryColor-300 transition flex items-center gap-1.5 mx-auto font-mono cursor-pointer"
        >
